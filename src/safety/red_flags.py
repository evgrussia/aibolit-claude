...
_IMMEDIATE_PATTERNS: list[tuple[str, str, str]] = [
    # Cardiac
    (r"(?:боль|давит|жжение|тяжесть)\s.*(?:груд|сердц|за\s?грудин)",
    ...
    # Neurological
    (r"(?:слабость|онемени|парез)\s.*(?:одн\w+\s+сторон|половин|лев\w+|прав\w+)",
...
_HIGH_PATTERNS: list[tuple[str, str, str]] = [
    (r"(?:давлени|ад)\s.*(?:18[0-9]|19[0-9]|2[0-9]{2})\s*/",
